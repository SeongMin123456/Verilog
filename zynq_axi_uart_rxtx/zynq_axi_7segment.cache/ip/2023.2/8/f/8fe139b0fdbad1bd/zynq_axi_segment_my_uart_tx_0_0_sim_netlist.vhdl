...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_my_uart_tx is
  ...
  attribute MARK_DEBUG of TX : signal is std.standard.true;
  ...
  signal \send_cnt[3]_i_5_n_0\ : STD_LOGIC;
...
\curr_state[0]_i_3\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"BFFFFFFF"
    )
        port map (
...
\send_cnt[1]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"BCA0"
    )
        port map (
...
\send_cnt[3]_i_5\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFE"
    )
        port map (
...
\serial[8]_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000FFFF02000000"
    )
        port map (
      I0 => send_cnt(3),
      I1 => send_cnt(2),
...
serial_inst: unisim.vcomponents.LUT1
    generic map(
      INIT => X"2"
    )
        port map (
